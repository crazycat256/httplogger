...
// PATCHED(httplogger): custom upstream WebSocket connect so wss:// can use
// BoringSSL (browser ClientHello mimic) instead of rustls/native-tls.
pub trait WebsocketIo: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send {}

impl<T> WebsocketIo for T where T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send {}

pub type WebsocketStream = tokio_tungstenite::WebSocketStream<Box<dyn WebsocketIo>>;

pub type WebsocketConnect = Arc<
    dyn Fn(
            Request<()>,
        ) -> Pin<
            Box<
                dyn Future<
                        Output = Result<(WebsocketStream, tungstenite::handshake::client::Response), tungstenite::Error>,
                    > + Send,
            >,
        > + Send
        + Sync,
