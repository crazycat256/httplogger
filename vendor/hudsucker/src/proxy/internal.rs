...
pub(crate) struct InternalProxy<C, CA, H, W> {
    ...
    // PATCHED(httplogger): custom upstream websocket connect (BoringSSL mimic).
    pub websocket_connect: Option<crate::WebsocketConnect>,
    ...
    W: Clone,
{
    fn clone(&self) -> Self {
        InternalProxy {
            ...
            websocket_connect: self.websocket_connect.clone(),
    ...
    W: WebSocketHandler,
{
    ...
    pub(crate) async fn proxy(
        ...
        if req.method() == Method::CONNECT {
            ...
            // PATCHED(httplogger): validate the custom upstream before replying 101.
            if self.websocket_connect.is_some() {
                Ok(self.upgrade_websocket_custom(req).await)
            } else {
    ...
    // PATCHED(httplogger): connect upstream first, propagate its response, and
    // decline unsupported compression instead of accepting broken RSV1 frames.
    #[instrument(skip_all)]
    async fn upgrade_websocket_custom(self, mut req: Request<Body>) -> Response<Body> {
        let mut upstream_req = match websocket_upstream_request(&req) {
            Some(req) => req,
            None => return bad_request(),
        };
        let uri = upstream_req.uri().clone();

        // tungstenite does not implement permessage-deflate. Declining it is
        // preferable to advertising it and then rejecting compressed frames.
        upstream_req
            .headers_mut()
        ...
        let connect = self
            .websocket_connect
            .as_ref()
            .expect("custom websocket connector")
            .clone();
        let (server_socket, upstream_res) = match connect(upstream_req).await {
            Ok(result) => result,
            Err(err) => return websocket_connect_error(err),
        };

        let (local_res, websocket) = match hyper_tungstenite::upgrade(&mut req, None) {
            Ok(result) => result,
        ...
        let fut = async move {
            match websocket.await {
                Ok(client_socket) => {
                    if let Err(err) = self
                        .bridge_websocket(client_socket, server_socket, uri)
                        .await
                    {
                        error!(error = &err as &dyn StdError, "Failed to handle WebSocket");
                    }
                }
                Err(err) => {
                    error!(
        ...
        let (_, body) = local_res.into_parts();
    ...
    async fn handle_websocket(
        ...
        // PATCHED(httplogger): custom upstream connect (BoringSSL ClientHello mimic)
        // replaces rustls/native-tls when `with_websocket_connect` is set.
        if let Some(connect) = &self.websocket_connect {
            let (server_socket, _) = connect(req).await?;
            return self.bridge_websocket(client_socket, server_socket, uri).await;
        }
        ...
        self.bridge_websocket(client_socket, server_socket, uri).await
    }

    // PATCHED(httplogger): shared bridge for stock and custom upstream transports.
    async fn bridge_websocket<S>(
        self,
        client_socket: WebSocketStream<TokioIo<Upgraded>>,
        server_socket: WebSocketStream<S>,
        uri: Uri,
    ) -> Result<(), tungstenite::Error>
    where
...
// PATCHED(httplogger): helpers for the upstream-first WebSocket handshake.
fn websocket_upstream_request(req: &Request<Body>) -> Option<Request<()>> {
    let mut parts = req.uri().clone().into_parts();
    parts.scheme = if parts.scheme.unwrap_or(Scheme::HTTP) == Scheme::HTTP {
        Some("ws".try_into().ok()?)
    } else {
        Some("wss".try_into().ok()?)
    };

    let mut upstream = Request::new(());
    *upstream.method_mut() = req.method().clone();
    *upstream.uri_mut() = Uri::from_parts(parts).ok()?;
    *upstream.version_mut() = req.version();
    *upstream.headers_mut() = req.headers().clone();
    Some(upstream)
}

fn websocket_connect_error(err: tungstenite::Error) -> Response<Body> {
    if let tungstenite::Error::Http(res) = err {
        let (parts, body) = res.into_parts();
        return Response::from_parts(parts, Body::from(body.unwrap_or_default()));
    }

    error!(error = &err as &dyn StdError, "Failed to connect upstream WebSocket");
    Response::builder()
