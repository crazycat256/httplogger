...
use hyper_util::{
    client::legacy::{connect::Connect, Builder as ClientBuilder},
    rt::TokioExecutor,
...
pub struct WantsHandlers<CA, C, H, W, F> {
    ...
    websocket_connect: Option<crate::WebsocketConnect>,
...
impl<CA, C, H, W, F> ProxyBuilder<WantsHandlers<CA, C, H, W, F>> {
    ...
    pub fn with_http_handler<H2: HttpHandler>(
        ...
        ProxyBuilder(WantsHandlers {
            ...
            websocket_connect: self.0.websocket_connect,
    ...
    /// PATCHED(httplogger): custom upstream WebSocket connect (used instead of
    /// rustls/native-tls when set).
    pub fn with_websocket_connect(self, connect: crate::WebsocketConnect) -> Self {
        ProxyBuilder(WantsHandlers {
            websocket_connect: Some(connect),
            ..self.0
        })
    }
